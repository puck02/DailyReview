...
test("chat model picker uses the full upstream model names", () => {
  ...
  assert.ok(app.includes("setModel(nextDefault);"));
  assert.doesNotMatch(app, /if \(current && current !== defaultModel && nextOptions\.includes\(current\)\) return current;/);
...
test("chat composer opens a pressure-sensitive handwriting pad", () => {
  ...
  assert.match(handwritingPad, /getCoalescedEvents/);
  assert.match(handwritingPad, /quadraticCurveTo\(previous\.x,\s*previous\.y,\s*endX,\s*endY\)/);
...
test("essay writing view renders a dedicated workspace and suggestion rail", () => {
  ...
  assert.ok(app.includes("const [topicImageDialogOpen, setTopicImageDialogOpen] = useState(false);"));
  assert.ok(app.includes("essay-topic-preview-button"));
  assert.ok(app.includes('aria-label="放大题图"'));
  ...
  assert.ok(app.includes('imageContextPreview.objective_description || "等待解析"'));
  ...
  assert.ok(app.includes("const [acceptedEssaySuggestion, setAcceptedEssaySuggestion] = useState"));
  assert.ok(app.includes("function acceptInlineEssaySuggestion"));
  ...
  assert.ok(app.includes("word_count: countEssayWords(draftRef.current),"));
  assert.ok(app.includes("paragraph_stage: deriveEssayParagraphStage(draftRef.current, cursorIndex),"));
  assert.ok(app.includes("{ signal: abortController.signal }"));
  assert.ok(app.includes("ref={essayEditorRef}"));
  assert.ok(app.includes("onKeyDown={handleEssayEditorKeyDown}"));
  assert.ok(app.includes("onSelect={updateEssayCursor}"));
  assert.ok(app.includes("onScroll={syncEssayGhostScroll}"));
  assert.ok(app.includes("essay-ghost-layer"));
  assert.ok(app.includes("essay-ghost-prefix"));
  assert.ok(app.includes("essay-ghost-suggestion"));
  assert.ok(app.includes("essay-undo-completion"));
  ...
  assert.match(styles, /\.essay-image-dialog-panel\s*{[^}]*position:\s*relative;/s);
  assert.match(styles, /\.essay-image-dialog-close\s*{[^}]*position:\s*absolute;/s);
  assert.match(styles, /\.essay-context-text\s*{[^}]*white-space:\s*pre-wrap;/s);
  assert.match(styles, /\.essay-ghost-layer\s*{[^}]*position:\s*absolute;[^}]*pointer-events:\s*none;/s);
  assert.match(styles, /\.essay-ghost-prefix\s*{[^}]*visibility:\s*hidden;/s);
  assert.match(styles, /\.essay-ghost-suggestion\s*{[^}]*color:\s*color-mix/s);
